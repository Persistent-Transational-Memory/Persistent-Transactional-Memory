 ...
 */
	...
	constexpr auto Workload = WorkloadType::SmallBank;

	/// Index Configuration
	constexpr auto DataIndex = IndexType::HashMap;

	/// Concurrent control Configuration
	constexpr auto ConcurrentControl = ConcurrentControlType::SP;

	/// Transaction Configuration
	constexpr auto TransactionManager = TransactionManagerType::PreloadStdTransactionManager;

	/// Storage Manager Configuration
	constexpr auto StorageManager = StorageManagerType::Simple_PMEMDATA_PMEMLOG;

	/// Strategy of thread binding
	constexpr ThreadBindStrategyType BindStrategy = ThreadBindStrategyType::HybridBind;

	/// Number of threads to be test
	const std::vector<uint32_t> test_thread_num{(BindStrategy == allocator::ThreadBindStrategy::HybridBind) ?
                                      std::vector<uint32_t>({24}) :
                                      std::vector<uint32_t>({24})};
#endif
...
int main() {
	...
	uint32_t test_time_millisecond = 5'000;
