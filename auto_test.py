...
THREAD_AMOUNT_TYPE_BIND_NUMA = [
    1, 8, 16, 24, 32, 40, 48
]

THREAD_AMOUNT_TYPE_CROSS_NUMA = [
    1, 8, 16, 24
]
...
if __name__ == '__main__':
    ...
    for i in range(len(sys.argv) - 1):
        run_env.append(sys.argv[i + 1])

    # Start test
    cmake_flags = ""
    cmake_flags += " -DAUTO_TEST=true"
    cmake_flags += " -DLOGGER_OUTPUT=FILE"
    ...
    for concurrent_control, \
        ...
        if thread_bind == "NUMABind":
            THREAD_AMOUNT_TYPE = THREAD_AMOUNT_TYPE_BIND_NUMA
        else:
            THREAD_AMOUNT_TYPE = THREAD_AMOUNT_TYPE_CROSS_NUMA
